...
import { TicketResponse, getTickets, resolveTicket } from '@/api/tickets';
...
export default function ViewTicketPage() {
  ...
  const handleDownloadAttachment = async (attachmentId: string, fileName: string) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/tickets/${ticketId}/attachments/${attachmentId}`, {
        method: 'GET',
        headers: {
  ...
  useEffect(() => {
    const fetchTicketDetails = async () => {
      ...
      try {
        ...
        // In a real implementation, you would fetch messages from an API endpoint
