...
import { FiAlertCircle, FiClock, FiBarChart2, FiCalendar, FiSearch, FiSliders } from 'react-icons/fi';
...
const ManagerSLAAlertsPage = () => {
  ...
  return (
    <div className="flex min-h-screen bg-slate-50">
      ...
      <main className={`flex-1 transition-all duration-300 ease-in-out ${sidebarOpen ? 'ml-[250px]' : 'ml-[80px]'}`}>
        <div className="p-6">
          ...
          <div className="mb-8 flex items-center">
            ...
            <div>
              <h1 className="text-2xl font-bold">SLA Management - Breach Alerts</h1>
              <p className="text-gray-600">Critical tickets that have breached Service Level Agreements</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-md p-4 mb-6">
            <div className="flex flex-col md:flex-row gap-4">
              ...
              <div className="flex items-center">
                <FiSliders className="text-gray-400 mr-2" />
                <select
            ...
            <div className="space-y-6">
              {filteredBreaches.length === 0 ? (
                <div className="ml-16 bg-white p-6 rounded-lg shadow-md text-center">
                  <p className="text-gray-600">No SLA breaches match your search criteria.</p>
                </div>
              ) : (
                filteredBreaches.map((breach) => (
                  <div key={breach.id} className="flex">
                    ...
                    <div className={`ml-4 flex-grow bg-white rounded-lg shadow-md border-l-4 ${getSeverityColor(breach.severity)}`}>
                      <div className="p-5">
                        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4">
                          <div>
                            <h3 className="text-lg font-semibold flex items-center">
                              Ticket {breach.ticketId}
                              <span className={`ml-2 px-2 py-1 text-xs rounded-full ${getSeverityBadge(breach.severity)}`}>
                                {breach.severity}
                              </span>
                            </h3>
                          ...
                          <div className="mt-2 md:mt-0">
                            <span className="text-red-600 font-medium flex items-center">
                              <FiClock className="mr-1" /> Breached {breach.timeElapsed} ago
                            </span>
                          </div>
                        </div>
                        
                        <div className="mb-4">
                          <p className="text-gray-700"><span className="font-medium">Issue:</span> {breach.issueDescription}</p>
                          <p className="text-gray-700"><span className="font-medium">Breach Time:</span> {breach.slaBreachTime}</p>
                          <p className="text-gray-700"><span className="font-medium">Escalation Level:</span> {breach.escalationLevel}</p>
                          <p className="text-gray-700"><span className="font-medium">Assigned To:</span> {breach.assignedTo}</p>
                        </div>
