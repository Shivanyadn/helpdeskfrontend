...
import { BarChart3, Clock, Star, CheckCircle, Calendar, TrendingUp, TrendingDown, Filter, Download, Printer, ChevronDown, Users, Award, Target, AlertCircle } from 'lucide-react';
...
const PerformanceReports = () => {
  ...
  const [monthlyBreakdownData, setMonthlyBreakdownData] = useState<MonthlyBreakdown[]>([]);
 // State for monthly breakdown data
  ...
  useEffect(() => {
    const fetchResolvedTickets = async () => {
      try {
        ...
        const response = await axios.get('https://help.zenapi.co.in/api/tickets/analytics', {
          headers: {
    ...
    const fetchResolutionTimes = async () => {
      try {
        ...
        const response = await axios.get('https://help.zenapi.co.in/api/tickets/resolution-times', {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
        ...
        if (response.data.success) {
          ...
          const totalResolutionTime = resolutionTimes.reduce(
            (sum: number, ticket: ResolutionTime) => {
              return sum + parseFloat(ticket.resolutionTimeInHours)/60;
            },
    ...
    const fetchMonthlyPerformanceData = async () => {
      try {
        ...
        if (analyticsResponse.data.success && resolutionTimesResponse.data.success) {
          ...
          const ticketsResolved = labels.map(() => parseFloat(performance.performanceScore) || 0);
    ...
    const fetchCategoryData = async () => {
      try {
        ...
        // Fetch tickets data
        const response = await axios.get('https://help.zenapi.co.in/api/tickets', {
          headers: { Authorization: `Bearer ${token}` },
        });
            ...
            <div className="bg-green-50 border border-green-200 rounded-xl p-5 flex items-start">
              ...
              <div>
                ...
                <p className="text-green-700 text-sm">
                  You are in the top 10% of agents for resolution time and customer satisfaction. Your SLA compliance is above the team average by 3%.
                </p>
                <div className="mt-3">
